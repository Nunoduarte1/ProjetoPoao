...
public class Venda {
    ...
    public float calculaPrecoCompras(){// penso que seja private pq so ira ser chamado aqui dentro para ser calculado o valor do preco total
        System.out.println("heyo the pizza here");
        ArrayList<Promocoes> promocoes=null;
        ...
        for (ProdutoNoCarrinho i: carrinhoCompras){
            ...
            if(promocoes!=null){
                ...
                for (Promocoes j:promocoes) {// ver se o que se le de ficheiros sai a null ou a zero se nao tiver la nenhum paramentro
                    System.out.println("dentro do for"+j.descontoValido(dataAtual));
    ...
    public String toString() {
        for (ProdutoNoCarrinho i: carrinhoCompras) {
            System.out.println(i.getQuantidade()+"-->"+i.getProduto());
        }
        System.out.println("valor da venda="+calculaCustoFinal());
        return "Compra realizada por " + cliente.getNome() + ", data da compra=" + dataAtual + '}';
    }
}